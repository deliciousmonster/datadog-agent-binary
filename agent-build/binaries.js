...
/** Where one binary comes from on one system, which `buildOn` can override per system. @param {AgentBinary} binary @param {Pick<Target, "os">} target @returns {BinarySource} */
export const sourceOf = (binary, target) =>
	binary.buildOn?.includes(target.os) ? "build" : binary.from;

/** The binaries this package compiles for one system. @param {Pick<Target, "os">} target @returns {readonly AgentBinary[]} */
export const builtFor = (target) =>
	binariesFor(target).filter((b) => sourceOf(b, target) === "build");

/** The binaries this package lifts out of Datadog's signed release for one system. @param {Pick<Target, "os">} target @returns {readonly AgentBinary[]} */
export const extractedFor = (target) =>
	binariesFor(target).filter((b) => sourceOf(b, target) === "release");

/** The binaries the base package carries: what every install gets. @param {Pick<Target, "os">} target @returns {readonly AgentBinary[]} */
export const baseBinaries = (target) =>
	binariesFor(target).filter((b) => !b.optional);

/** The binaries the opt-in probe package carries, which an operator installs by name. @param {Pick<Target, "os">} target @returns {readonly AgentBinary[]} */
export const probeBinaries = (target) =>
	binariesFor(target).filter((b) => b.optional);

/** The binaries that exist for one system, which is not always all of them. @param {Pick<Target, "os">} target @returns {readonly AgentBinary[]} */
...
/** The Go build tags recorded in a linked binary, or null when it carries no build-info record at all. @param {Buffer} bytes @returns {string[] | null} */
...
/** The name a binary is shipped and copied under for one target, e.g. `datadog-agent.exe`. @param {AgentBinary} binary @param {Target} target @returns {string} */
