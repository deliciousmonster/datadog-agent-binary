...
const { DatadogAgentDownloader } = require(
	path.join(sandbox, "dist", "downloader.js")
);
...
test("a missing pin file is fatal, not a fallback to upstream latest", async () => {
	...
	await assert.rejects(
		...
		(error) =>
			error.message.includes(pinPath) &&
			/must not\s+silently float/.test(error.message),
...
test("a clone describing past the tag is refused", () => {
	...
	assert.throws(
		() => downloader["assertCheckoutMatches"]("7.60.1", "7.60.1-3-gabc1234"),
...
test("a clone describing as a bare sha is refused", () => {
	...
	assert.throws(
		...
		(error) =>
			error.message.includes("7.60.1") && error.message.includes("abc1234"),
...
test("an exact tag match is the only accepted checkout", () => {
	const downloader = new DatadogAgentDownloader();
	assert.doesNotThrow(() =>
		downloader["assertCheckoutMatches"]("7.60.1", "7.60.1")
	);
