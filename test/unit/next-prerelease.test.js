...
const {
	...
	compare,
	parseVersion,
} = require("../../scripts/next-prerelease.js");
...
test("never goes backwards from an already-released version", () => {
	...
	assert.ok(
		compare(parseVersion(version, "next"), highest) > 0,
		`${version} must sort above 7.75.6-next.1`
	);
});
...
test("a bad channel is rejected", () => {
	assert.throws(
		() => nextPrerelease({ channel: "next.1", tags: ["v1.0.0"] }),
		/lowercase alphanumeric/
	);
	assert.throws(
		() => nextPrerelease({ channel: "", tags: ["v1.0.0"] }),
		/lowercase alphanumeric/
	);
});
