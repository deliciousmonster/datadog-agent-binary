...
test("E404 means never published; anything else is transient", () => {
	assert.strictEqual(
		classifyNpmFailure("npm error code E404"),
		"not-published"
	);
	assert.strictEqual(
		classifyNpmFailure("npm error 404 Not Found"),
		"not-published"
	);
	assert.strictEqual(
		classifyNpmFailure("npm error network ECONNRESET"),
		"transient"
	);
...
test("without a token, all names published passes with the unprovable residue stated", () => {
	const result = checkAuthPath(
		PKG,
		"",
		fakeExec({ published: packageNames(PKG) })
	);
	assert.strictEqual(result.ok, true);
