...
import { clearStaleHarperPidFiles } from "../../runtime/supervisor.js";
import { withTempDir } from "../support/sandbox.js";
import { captureLogs } from "../support/loopback.js";

const pidFile = (root, name) => path.join(root, "pids", `${name}.pid`);
const write = (root, name, pid) => {
...
test("NEGATIVE: a Harper pid file naming a live pid that is not the agent is removed, and the log says what it was running", () =>
	withTempDir("dd-hpid-", async (root) => {
		...
		assert.equal(
			fs.existsSync(pidFile(root, "datadog-trace-agent")),
			false,
			"the stale file survived"
		);
		const line = lines.find((entry) => entry.includes("removed"));
		assert.ok(line, "nothing was logged about the removal");
		assert.match(
			line,
			new RegExp(`named pid ${process.pid}, which is running`)
		);
	}));
	...
	withTempDir("dd-hpid-", async (root) => {
		...
		const lines = await captureLogs(() =>
			clearStaleHarperPidFiles(
				root,
				[
					{ name: "datadog-agent", argv: [process.argv[0]] },
		...
		assert.equal(
			fs.existsSync(pidFile(root, "datadog-agent")),
			true,
			"the real process's file was removed"
		);
		assert.equal(
...
test("no root, nothing to clear", async () => {
	const lines = await captureLogs(() =>
		clearStaleHarperPidFiles(null, [{ name: "x", argv: ["y"] }], console)
	);
	assert.deepEqual(lines, []);
});
