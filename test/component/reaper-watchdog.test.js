...
import { keepReaperAlive } from "../../runtime/supervisor.js";
...
describe("keeping a reaper on the node", () => {
	...
	const watcher = (relaunch, log = quiet) =>
		keepReaperAlive({
			pidDir: dir,
	...
	it("relaunches when the lock names a pid nothing holds", async () => {
		...
		const w = watcher(async () => {
			called++;
			lock(child.pid); // what a successful relaunch leaves behind
		});
	...
	it("backs off when a relaunch leaves no reaper, rather than spawning every minute", async () => {
		...
		assert.equal(
			await w.tick(),
			"backoff",
			"the next tick must not spawn again immediately"
		);
		assert.match(lines[0], /left none running/);
		assert.match(lines[0], /next attempt in/);
		w.stop();
	});
	...
	it("treats a throwing relaunch as a failure and says so", async () => {
		...
		const w = watcher(
			...
			{ info() {}, warn() {}, error: (line) => lines.push(line) }
		);
		assert.equal(await w.tick(), "failed");
	...
	it("NEGATIVE: does not start a second relaunch while one is still running", async () => {
		...
		assert.equal(
			peak,
			1,
			"two threads' worth of spawning is what the lock exists to prevent"
		);
