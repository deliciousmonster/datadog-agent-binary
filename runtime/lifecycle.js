...
//
// Both are generic. Nothing here names a consumer: a component that is imported for its resources and
// never called is a Harper-shaped failure every plugin can hit, and the once-per-thread guarantee below is
// the same for any component that starts something. This is the second piece that belongs in the guard.
 ...
 */
export function watchForNeverCalled({
	log,
	label,
	configEntry,
	deadlineMs = START_DEADLINE_MS,
}) {
 ...
 *
 * @param {object} options
 * @param {() => Promise<object>} options.start
