// Everything resources.js used to hold. It is a factory rather than a module of constants because
// resources.js is re-evaluated per test with a cache-busting query string, and the ports are read from the
// environment: state at this module's scope would be shared across those evaluations and a port test would
// bind the first run's numbers forever. Called once per evaluation, every reference below is fresh.
//
// `spawn` and Harper's compartment globals arrive as arguments. resources.js is the only file Harper
// compiles, so it is the only place they can be read; a module that imports `spawn` itself gets the
...
import { PACKAGE_NAME, resolveBinary, resolveEbpfDir } from "./binary.js";
...
import { untraceAgentProbes } from "./probe.js";
import { probePrivilege } from "./system-probe.js";
import {
	currentReaper,
	nodeProcess,
	supervisorFor,
...
export function datadog({ spawn, logger, Resource, processes }) {
	const host = logger ?? console;
	const ResourceBase = Resource ?? class {};

	// Every method on Harper's Logger is declared optional. Normalised once here rather than defended per
	// call, because the guard calls ctx.log.info and ctx.log.warn unguarded after it has committed a lock.
	const channel =
		(...names) =>
		(message) =>
			(
	...
	// Read once per instance, because every worker thread renders the config and probes the endpoints from
	// the same numbers and a second reading could disagree with the first.
	const ports = {
	...
	// Every URL this component polls. probe.js already suppresses these at the call site; this is the public
	// half, and it only holds until some other caller reconfigures the same plugins.
	const PROBE_URLS = [
	...
	if (tracer) {
		try {
			...
			// A shape untraceAgentProbes did not expect from tracer.use(): unlike a missing require, this
			// leaves the probes untraced, so it gets its own log line rather than sharing the silent path.
			log.error(
	...
	const CONFIG_ENTRY = `${basename(COMPONENT_DIR)}: { package: "${PACKAGE_NAME}" }`;
	 ...
	 *
	 * Reported rather than enforced. The binary is the authority on whether it can load an eBPF program, so
	 * a refusal here would be this component overruling it on a heuristic. What this replaces is a restart
	 * loop whose logs say `operation not permitted` and nothing about which capability is missing.
	 */
	function probeStatus(probes, ebpfDir) {
		const reasons = [];
		if (probes.systemProbe) {
			const privilege = probePrivilege();
			// `able: null` is Windows: unknown rather than refused, because nothing here can tell whether
			// the drivers are installed without opening one. Reported as a blocker either way, since an
			// operator who has not installed them needs to read it.
	...
	/** Per worker thread, set by handleApplication; a request that beats it reads NOT_STARTED. */
	let supervisor;
	/** Where the guard's locks live, kept for the read path: the reaper's is re-read on every status. */
	let pidDir;
	/** The trace-agent's log: its refusal lines are the only trace-hop evidence this agent build gives. */
	let traceLogPath;
	/** Each agent's own verifier, by name, so the read path can retake a verdict a restart made stale. */
	let verifiers = new Map();
	/** This thread's series timer, kept so a second startup cannot leave two of them running. */
	let series;
	/** What startup left running on this thread, read by the series timer rather than captured by it. */
	let startedProcesses = [];

	/** The fields every status shape starts from, so NOT_STARTED and startAgents cannot drift apart. */
	const baseStatus = () => ({
	 ...
	 *
	 * The members are read per tick from the status this startup produced, through `nodeProcess`, so a pid
	 * the guard replaced under chaos is measured as the process the node runs now rather than the one it
	 * started. Only the claim holder sends; every other thread's timer costs a file read.
	 */
	function scheduleProcessSeries(dir, confd) {
		...
		// the operator saying they intend the real check to, so this stands down rather than becoming a
		// second source.
	...
	async function startAgents(scope) {
		...
		const status = { supervision: supervision.kind, ...baseStatus() };
		try {
			...
			// The credentials ride in the inherited environment, invisible to the config contents, so a
			// rotated key must be folded in here or a thread joins the agent still posting under the old one.
	...
	// The one failure this component cannot see from inside: Harper imports it for its resources and never
	// calls the plugin, which is what an auto-scanned component directory gets. Module evaluation is the
	// only vantage point left.
	...
	function handleApplication(scope) {
		...
		// A deploy pre-flight loads the component against a live node just to validate it; starting agents
		// there re-enters the sweep and spawn path on every `harper deploy`.
	...
	class DatadogStatus extends ResourceBase {
		static async get() {
			// The counters belong to the node's trace-agent, not to this thread, so they are read whether or
			// not this thread is the one that started it.
