...
/**
 * Log sources for what runs on this node: Harper's own log (`logging.root` is `log` under the root path
 * unless a node moved it) and the two agents' and the reaper's, which are the first thing an operator
 * wants when the node stops reporting. Tailed only while logs are enabled (DD_LOGS_ENABLED=true).
 */
function renderLogSources(harperLog, paths) {
	...
	return [
		...
		"logs:",
		...source(harperLog, "harper", "harper"),
