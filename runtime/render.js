...
export function renderSecurityAgentYaml(paths, resolved) {
	...
	return [
		"# GENERATED by resources.js on every Harper worker start. Edits are overwritten.",
		"log_to_console: false",
		// `security_agent.log_file`, not the top-level key. The security-agent reads this file as a main
		// config, so a top-level `log_file` sets the CORE agent's path and it resolves correctly while the
		// security-agent goes on writing /var/log/datadog/security-agent.log, its compiled-in default. That
		// directory is root-owned and outside the runtime tree, so the one log an operator wants when runtime
		// security misbehaves was the one log this component did not place. The rolling limits below are
		// global and do apply.
		"security_agent:",
