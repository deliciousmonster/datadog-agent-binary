...
| Is it receiving anything? | `curl -sk https://127.0.0.1:5012/debug/vars \| jq '[.receiver[].SpansReceived] \| add'`, or `grep 'traces received' <runtime>/logs/trace-agent.log` at **debug** level | A climbing count is direct proof of receipt. Zero or absent means the tracer is not sending: confirm `dd-trace` is loaded in the application process, not merely installed. The receiver array is a one-minute window that empties when nothing arrives. |
| Is it delivering? | `curl -sk https://127.0.0.1:5012/debug/vars \| jq .stats_writer.Payloads` | Above zero means the intake accepted an authenticated payload. **Do not read `Traces: 0 payloads` off `datadog-agent status`**: it renders `trace_writer`, which is zero on 7.73.0 through at least 7.82.1 whatever the agent is doing, because two writers race for one expvar slot and the idle one usually wins. |
