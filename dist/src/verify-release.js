...
import { DATADOG_APT_BASE, DATADOG_APT_COMPONENT, DATADOG_APT_FINGERPRINT, DATADOG_APT_SUITE, } from "./release.js";
export const sha256 = (bytes) => createHash("sha256").update(bytes).digest("hex");
/** URL of the signed Release file and its detached signature, for one suite. */
export const releaseUrls = (base = DATADOG_APT_BASE, suite = DATADOG_APT_SUITE) => ({
    release: `${base}/dists/${suite}/Release`,
    signature: `${base}/dists/${suite}/Release.gpg`,
});
export const packagesUrl = (debArch, base = DATADOG_APT_BASE, suite = DATADOG_APT_SUITE, component = DATADOG_APT_COMPONENT) => `${base}/dists/${suite}/${component}/binary-${debArch}/Packages`;
export const artifactUrl = (artifact, base = DATADOG_APT_BASE) => `${base}/${artifact.path}`;
/**
...
export function hashFromPackages(packages, poolPath) {
    for (const stanza of packages.split(/\n\n+/)) {
        if (!new RegExp(`^Filename:\\s*${escapeRe(poolPath)}\\s*$`, "m").test(stanza))
            continue;
 ...
 */
export function verifyRelease({ artifact, bytes, release, packages, signature, }) {
    const failures = [];
    const checked = [];
    if (!signature.good)
        failures.push("the Release file is not signed by a key gpg trusts, so nothing below it can be believed");
    else if (normaliseFingerprint(signature.fingerprint) !== DATADOG_APT_FINGERPRINT)
        failures.push(`the Release file is signed by ${signature.fingerprint ?? "an unnamed key"}, not by Datadog's ` +
            `APT key ${DATADOG_APT_FINGERPRINT}`);
    else
        checked.push("Release signed by Datadog's APT key");
    ...
    if (!wantIndex)
        ...
        failures.push(`${indexPath} hashes to ${gotIndex}, and the signed Release says ${wantIndex}`);
    else
        checked.push(`${indexPath} matches the signed Release`);
    ...
    if (!wantDeb)
        ...
        failures.push(`the pin for ${artifact.path} is ${artifact.sha256}, and the repository says ${wantDeb}. ` +
            `Either the pin is stale or the artefact was republished; do not extract from it until that is settled`);
    else
        checked.push(`${artifact.path} matches the pin and the repository`);
    const gotDeb = sha256(bytes);
    if (gotDeb !== artifact.sha256)
        failures.push(`the downloaded ${artifact.path} hashes to ${gotDeb}, not the pinned ${artifact.sha256}`);
        ...
        failures.push(`the downloaded ${artifact.path} is ${bytes.byteLength} bytes, not the pinned ${artifact.size}`);
    else
        checked.push("the downloaded artefact matches the pin");
    return { ok: failures.length === 0, failures, checked };
}
/** gpg prints a fingerprint with no separators; a human writing one down uses spaces. Compare neither way. */
const normaliseFingerprint = (value) => (value ?? "").replace(/\s+/g, "").toUpperCase();
/**
...
//# sourceMappingURL=verify-release.js.map