...
import { fetchAgentSource, fetchLatestVersion, pinnedVersion, } from "./downloader.js";
...
async function linkIntoGoPath(goPath, sourceDir) {
    ...
    await stat(link).catch(() => symlink(relative(goSrcDir, sourceDir), link, "dir"));
}
...
export async function buildAgents(request) {
    ...
    const resolved = version ?? (await pinnedVersion()) ?? (await fetchLatestVersion());
