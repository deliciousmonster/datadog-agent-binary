...
import { buildAgents, currentTarget, fetchLatestVersion, targetNames, } from "./index.js";
// One exit path: commands throw, and the failure is reported here rather than in each action.
const run = (action) => async (options) => {
    try {
...
program
    ...
    .action(run(async () => {
    logger.info(`Latest Datadog Agent version: ${await fetchLatestVersion()}`);
}));
program.parse();
