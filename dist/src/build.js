 ...
 */
const BUILD_TIMEOUT_MS = 2700000;
...
function run(command, args, cwd, env, options = {}) {
    ...
    return new Promise((fulfil, reject) => {
        ...
        child.on("close", (code, signal) => code === 0
            ? fulfil(output)
            : reject(new Error(`${command} ${args.join(" ")} ${signal
...
const PYTHON_PROBE = "import sys;print('%d.%d' % sys.version_info[:2], sys.executable)";
/**
...
export async function pipxInterpreter(pin, probe) {
    ...
    for (const candidate of ["python3", "python"]) {
        ...
        if (executable && atLeast(version, pin))
...
async function ensureDda(cwd, env) {
    try {
        ...
        const interpreter = await pipxInterpreter(await pythonPin(cwd), (candidate) => run(candidate, ["-c", PYTHON_PROBE], cwd, env, {
            capture: true,
            timeoutMs: PROBE_TIMEOUT_MS,
        }));
        await run("pipx", ["install", "dda", ...interpreter], cwd, env).catch(() => run("pip", ["install", "dda"], cwd, env));
    }
}
...
export async function resolveWindowsShell(candidates) {
    ...
    throw new Error(`No MSYS2 bash found for bazel. Looked at: ${candidates.join(", ")}. ` +
...
export async function writeBazelShellOverride(sourceDir, shell) {
    ...
    await writeFile(join(sourceDir, "user.bazelrc"), "# Written by @deliciousmonster/datadog-agent-binary. .bazelrc points both of these at\n" +
...
async function windowsPreconditions(sourceDir) {
    await writeBazelShellOverride(sourceDir, await resolveWindowsShell(windowsShellCandidates()));
...
export async function prepareHost(target, sourceDir) {
    ...
    if (toolchain.GOTOOLCHAIN)
        logger.info(`Building with ${toolchain.GOTOOLCHAIN}, which the release pins`);
    else if (pin)
        logger.warn(`.go-version reads "${pin}", which is not a version; the toolchain is whatever go.mod resolves`);
    return {
        ...toolchain,
        ...(await cacheHome()),
...
export async function build({ target, sourceDir, outputDir, }) {
    ...
    if (target.precondition) {
        ...
        await run(command, args, sourceDir, base).catch(() => {
            throw new Error(`${target.os} requires \`${target.precondition}\` to succeed first`);
        });
    }
    ...
    await run("dda", ["--no-interactive", "inv", "install-tools"], sourceDir, env);
    const shipped = [];
    ...
    for (const binary of builtFor(target)) {
        ...
        await run("dda", ["--no-interactive", "inv", binary.task, ...buildArgs(binary)], sourceDir, env);
...
async function stripBinary(file, target, env) {
    ...
    const strip = target.os === "macos" ? ["-S", file] : ["--strip-unneeded", file];
    try {
        await run("strip", strip, dirname(file), env);
    }
    catch (error) {
        logger.warn(`Could not strip ${basename(file)}, shipping it with its debug symbols: ${error instanceof Error ? error.message : String(error)}`);
        return;
    }
    const after = (await stat(file)).size;
    logger.info(`Stripped ${basename(file)}: ${before} -> ${after} bytes, ${Math.round(((before - after) / before) * 100)}% smaller`);
}
