...
const RELEASES = "https://api.github.com/repos/DataDog/datadog-agent/releases/latest";
/** The pinned release, so a package cannot be labelled one version and built from another. */
export async function pinnedVersion() {
    try {
        return ((await readFile(join(process.cwd(), ".datadog-agent-version"), "utf8")).trim() || undefined);
    }
    catch {
        return undefined;
    }
...
export async function fetchAgentSource(version, into) {
    ...
    execFileSync("git", ["clone", "--depth", "1", "--branch", version, REPO, into], {
        stdio: ["inherit", "pipe", "inherit"],
    });
    const described = execFileSync("git", ["-C", into, "describe", "--tags", "--always"], {
        encoding: "utf8",
        stdio: ["inherit", "pipe", "inherit"],
    });
